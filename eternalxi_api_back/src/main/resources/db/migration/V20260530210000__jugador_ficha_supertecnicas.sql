-- Ficha ampliada de jugador + catálogo de supertécnicas reutilizable.
-- supertecnicas: definición única (varios jugadores pueden compartir la misma).
-- jugador_supertecnica: enlace jugador ↔ supertécnica (4 slots por jugador).

ALTER TABLE jugadores
    ADD COLUMN pais VARCHAR(64) NULL COMMENT 'País de origen, p. ej. Francia' AFTER genero,
    ADD COLUMN altura SMALLINT UNSIGNED NULL COMMENT 'Altura en centímetros, p. ej. 174' AFTER pais,
    ADD COLUMN estilo ENUM('PICARO', 'PRECISO', 'POTENTE', 'VALIENTE', 'AGIL') NULL COMMENT 'Estilo de juego del jugador' AFTER altura;

CREATE TABLE IF NOT EXISTS supertecnicas (
    id BIGINT NOT NULL AUTO_INCREMENT,
    nombre VARCHAR(128) NOT NULL,
    potencia TINYINT UNSIGNED NOT NULL COMMENT 'Potencia numérica (p. ej. 25-55)',
...
CREATE TABLE IF NOT EXISTS jugador_supertecnica (
    ...
    orden TINYINT UNSIGNED NOT NULL COMMENT 'Slot 1-4 del jugador',
    creado_en TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    PRIMARY KEY (id),
