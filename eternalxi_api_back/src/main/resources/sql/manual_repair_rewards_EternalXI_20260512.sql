-- =============================================================================
-- EternalXI — Reparación manual esquema recompensas (remoto sin Flyway)
-- Fecha: 2026-05-12
--
-- IMPORTANTE
--  - Este fichero = VARIANTE A (con FOREIGN KEY). La variante B está en:
--      manual_repair_rewards_EternalXI_20260512_NO_FK.sql
--    Ejecute UNO de los dos, no ambos (mismas semillas al final; ON DUPLICATE es idempotente).
--  - DELIMITER es del cliente mysql.exe. En MySQL Workbench puede fallar al pegar:
--      use la opción de delimitador del editor o ejecute desde: mysql -u... -p < este_archivo.sql
--  - Tipos: se asume INT para PK/FK alineado con tu legado (liga_participante_cartas
--    antigua con INT). Si SHOW CREATE TABLE de ligas / participantes / jornadas
--    muestra BIGINT, sustituya INT por BIGINT en las columnas FK de esta script
--    antes de ejecutar la variante A.
--  - Si ya ejecutó la mitad de A y falló: revise tablas vacías nuevas, DROP
--    manual si procede, y ejecute B (o corrija tipos y repita A).
-- =============================================================================

-- ---------------------------------------------------------------------------
-- BLOQUE COMÚN (siempre primero)
-- ---------------------------------------------------------------------------

USE EternalXI;

-- 1) usuario_recursos.puntos_recompensa
SET @db := DATABASE();
SET @q := IF(
...
-- 2–3) Renombrar liga_participante_cartas legada (id_carta → cartas) solo si:
--     existe la tabla, NO tiene columna id_definicion_carta (esquema viejo),
--     está vacía, y NO existe ya el backup con el nombre fijo.
...
proc_exit: BEGIN
    ...
    IF v_legacy > 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Ya existe liga_participante_cartas_legacy_empty_20260512. DROP si vacía y no la necesita, o renómbrela, y re-ejecute.';
    END IF;
...
-- ---------------------------------------------------------------------------
-- VARIANTE A — Con FOREIGN KEYS (tipos INT en FK; ajustar a BIGINT si hace falta)
-- NO ejecute la VARIANTE B si esta se aplicó correctamente.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- SEMILLAS definiciones_carta (ejecutar una vez; idempotente por codigo)
-- ---------------------------------------------------------------------------
...
-- =============================================================================
-- Si la variante A falló (p. ej. errno 150), use manual_repair_rewards_EternalXI_20260512_NO_FK.sql
-- tras limpiar tablas vacías creadas a medias si hace falta. No re-ejecute los CREATE INDEX si ya existen.
-- =============================================================================
